...
def process_vm(resource): #! дополнить диски и GPU
    ...
    if(platform_id == "standard-v3-t4"):
        ...
        if(preemptible == True):
            # cpu
            add_usage("dn2g8mst5iopbv5611le", cores, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. 100% vCPU \u2014 preemptible instances [core*hour]
            # ram
            add_usage("dn287rnoq7s09qnpqg78", memory, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. RAM - preemptible instances [gbyte*hour]
            # gpu
            add_usage("dn2tsmg9n2tp1bh198b1", gpus, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. GPU - preemptible instances [gbyte*hour]
            
        # non preemptible
        if(preemptible == False):
            # cpu
            add_usage("dn27gp9ehd967jgq0pfg", cores, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. 100% vCPU [core*hour]
            # ram
            add_usage("dn2hu7niuumq0pp612m3", memory, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. RAM" [gbyte*hour]
            # gpu
            add_usage("dn254fsua0fbspk0n19j", gpus, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. GPU [gbyte*hour]

    if(platform_id == "gpu-standard-v3"):
        # preemptible
        if(preemptible == True):
            # cpu
            add_usage("dn2se131g38tgbdtknj4", cores, resource_name, resource_type) # AMD Epyc with Nvidia A100. 100% vCPU - preemptible instances [core*hour]
            # ram
            add_usage("dn213pboth9buu4ig6t9", memory, resource_name, resource_type) # AMD Epyc with Nvidia A100. RAM \u2014 preemptible instances [gbyte*hour]
            # gpu
            add_usage("dn2a93qq9mc45ndq7qq9", gpus, resource_name, resource_type) # AMD Epyc with Nvidia A100. GPU - preemptible instances [gbyte*hour]
            
        # non preemptible
        if(preemptible == False): 
            # cpu
            add_usage("dn2tvhok96voc090hndj", cores, resource_name, resource_type) # AMD Epyc with Nvidia A100. 100% vCPU [core*hour]
            # ram
            add_usage("dn24un5470j0l5rbsahu", memory, resource_name, resource_type) # AMD Epyc with Nvidia A100. RAM [gbyte*hour]
            # gpu
            add_usage("dn23bfec1dkv6qmeipm7", gpus, resource_name, resource_type) # AMD Epyc with Nvidia A100. GPU [gbyte*hour]
    
    if(platform_id == "gpu-standard-v2"):
        # preemptible
        if(preemptible == True):
            # cpu
            add_usage("dn2so61be1pud0fg4mrp", cores, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. 100% vCPU - preemptible instances [core*hour]
            # ram
            add_usage("dn2njho0re0917mom5i6", memory, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. RAM \u2014 preemptible instances [gbyte*hour]
            # gpu
            add_usage("dn2639t97vtchqueq709", gpus, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. GPU - preemptible instances [gbyte*hour]
            
        # non preemptible
        if(preemptible == False):
            # cpu
            add_usage("dn23oh47d817get24dru", cores, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. 100% vCPU [core*hour]
            # ram
            add_usage("dn2qiqi4104s7o7eatbp", memory, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. RAM [gbyte*hour]
            # gpu
            add_usage("dn2ov44offujr3euo48d", gpus, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. GPU [gbyte*hour]

    if(platform_id == "gpu-standard-v1"):
        # preemptible
        if(preemptible == True):
            # cpu
            add_usage("dn2g7rssjjnmbl5a3n9b", cores, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. 100% vCPU - preemptible instances [core*hour]
            # ram
            add_usage("dn2aohdunrvmqp68d0bf", memory, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. RAM - preemptible instances [gbyte*hour]
            # gpu
            add_usage("dn2pr6ebg1947fu35t7a", gpus, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. GPU - preemptible instances [gbyte*hour]
            
        # non preemptible
        if(preemptible == False):
            # cpu
            add_usage("dn2fc012e2f1tkd5naci", cores, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. 100% vCPU [core*hour]
            # ram
            add_usage("dn25k0fsmikvpi2nbj2m", memory, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. RAM [gbyte*hour]
            # gpu
            add_usage("dn2kc7kdofj2bh3boqjd", gpus, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. GPU [gbyte*hour]
    ...
    if(boot_disk_size > 0):
        ...
        if(boot_disk_type == "network-ssd-io-m3"): #! нет в биллинге
            add_usage("xxx", boot_disk_size, resource_name, resource_type) # xxx [gbyte*hour]
...
def process_vm_disk(resource):
    ...
    if(disk_size > 0):
        ...
        if(disk_type == "network-ssd-io-m3"): #! нет в биллинге
            add_usage("xxx", disk_size, resource_name, resource_type) # xxx [gbyte*hour]
...
def process_vm_group(resource):
    ...
    if(platform_id == "standard-v3-t4"):
        ...
        if(preemptible == True):
            # cpu
            add_usage("dn2g8mst5iopbv5611le", cores * instance_num, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. 100% vCPU \u2014 preemptible instances [core*hour]
            # ram
            add_usage("dn287rnoq7s09qnpqg78", memory * instance_num, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. RAM - preemptible instances [gbyte*hour]
            # gpu
            add_usage("dn2tsmg9n2tp1bh198b1", gpus * instance_num, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. GPU - preemptible instances [gbyte*hour]
            
        # non preemptible
        if(preemptible == False):
            # cpu
            add_usage("dn27gp9ehd967jgq0pfg", cores * instance_num, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. 100% vCPU [core*hour]
            # ram
            add_usage("dn2hu7niuumq0pp612m3", memory * instance_num, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. RAM" [gbyte*hour]
            # gpu
            add_usage("dn254fsua0fbspk0n19j", gpus * instance_num, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. GPU [gbyte*hour]
    
    if(platform_id == "gpu-standard-v3"):
        # preemptible
        if(preemptible == True):
            # cpu
            add_usage("dn2se131g38tgbdtknj4", cores * instance_num, resource_name, resource_type) # AMD Epyc with Nvidia A100. 100% vCPU - preemptible instances [core*hour]
            # ram
            add_usage("dn213pboth9buu4ig6t9", memory * instance_num, resource_name, resource_type) # AMD Epyc with Nvidia A100. RAM \u2014 preemptible instances [gbyte*hour]
            # gpu
            add_usage("dn2a93qq9mc45ndq7qq9", gpus * instance_num, resource_name, resource_type) # AMD Epyc with Nvidia A100. GPU - preemptible instances [gbyte*hour]
            
        # non preemptible
        if(preemptible == False): 
            # cpu
            add_usage("dn2tvhok96voc090hndj", cores * instance_num, resource_name, resource_type) # AMD Epyc with Nvidia A100. 100% vCPU [core*hour]
            # ram
            add_usage("dn24un5470j0l5rbsahu", memory * instance_num, resource_name, resource_type) # AMD Epyc with Nvidia A100. RAM [gbyte*hour]
            # gpu
            add_usage("dn23bfec1dkv6qmeipm7", gpus * instance_num, resource_name, resource_type) # AMD Epyc with Nvidia A100. GPU [gbyte*hour]

    if(platform_id == "gpu-standard-v2"):
        # preemptible
        if(preemptible == True):
            # cpu
            add_usage("dn2so61be1pud0fg4mrp", cores * instance_num, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. 100% vCPU - preemptible instances [core*hour]
            # ram
            add_usage("dn2njho0re0917mom5i6", memory * instance_num, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. RAM \u2014 preemptible instances [gbyte*hour]
            # gpu
            add_usage("dn2639t97vtchqueq709", gpus * instance_num, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. GPU - preemptible instances [gbyte*hour]
            
        # non preemptible
        if(preemptible == False):
            # cpu
            add_usage("dn23oh47d817get24dru", cores * instance_num, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. 100% vCPU [core*hour]
            # ram
            add_usage("dn2qiqi4104s7o7eatbp", memory * instance_num, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. RAM [gbyte*hour]
            # gpu
            add_usage("dn2ov44offujr3euo48d", gpus * instance_num, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. GPU [gbyte*hour]

    if(platform_id == "gpu-standard-v1"):
        # preemptible
        if(preemptible == True):
            # cpu
            add_usage("dn2g7rssjjnmbl5a3n9b", cores * instance_num, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. 100% vCPU - preemptible instances [core*hour]
            # ram
            add_usage("dn2aohdunrvmqp68d0bf", memory * instance_num, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. RAM - preemptible instances [gbyte*hour]
            # gpu
            add_usage("dn2pr6ebg1947fu35t7a", gpus * instance_num, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. GPU - preemptible instances [gbyte*hour]
            
        # non preemptible
        if(preemptible == False):
            # cpu
            add_usage("dn2fc012e2f1tkd5naci", cores * instance_num, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. 100% vCPU [core*hour]
            # ram
            add_usage("dn25k0fsmikvpi2nbj2m", memory * instance_num, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. RAM [gbyte*hour]
            # gpu
            add_usage("dn2kc7kdofj2bh3boqjd", gpus * instance_num, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. GPU [gbyte*hour]
    ...
    if(boot_disk_size > 0):
        ...
        if(boot_disk_type == "network-ssd-io-m3"): #! нет в биллинге
            add_usage("xxx", boot_disk_size * instance_num, resource_name, resource_type) # xxx [gbyte*hour]
...
def process_kube_node(resource):
    ...
    if(platform_id == "standard-v3-t4"):
        ...
        if(preemptible == True):
            # cpu
            add_usage("dn2g8mst5iopbv5611le", resources_cores*instance_num, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. 100% vCPU \u2014 preemptible instances [core*hour]
            # ram
            add_usage("dn287rnoq7s09qnpqg78", resources_memory*instance_num, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. RAM - preemptible instances [gbyte*hour]
            # gpu
            add_usage("dn2tsmg9n2tp1bh198b1", resources_gpus*instance_num, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. GPU - preemptible instances [gbyte*hour]
            
        # non preemptible
        if(preemptible == False):
            # cpu
            add_usage("dn27gp9ehd967jgq0pfg", resources_cores*instance_num, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. 100% vCPU [core*hour]
            # ram
            add_usage("dn2hu7niuumq0pp612m3", resources_memory*instance_num, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. RAM" [gbyte*hour]
            # gpu
            add_usage("dn254fsua0fbspk0n19j", resources_gpus*instance_num, resource_name, resource_type) # Intel Ice Lake with Nvidia T4. GPU [gbyte*hour]
    
    if(platform_id == "gpu-standard-v3"):
        # preemptible
        if(preemptible == True):
            # cpu
            add_usage("dn2se131g38tgbdtknj4", resources_cores*instance_num, resource_name, resource_type) # AMD Epyc with Nvidia A100. 100% vCPU - preemptible instances [core*hour]
            # ram
            add_usage("dn213pboth9buu4ig6t9", resources_memory*instance_num, resource_name, resource_type) # AMD Epyc with Nvidia A100. RAM \u2014 preemptible instances [gbyte*hour]
            # gpu
            add_usage("dn2a93qq9mc45ndq7qq9", resources_gpus*instance_num, resource_name, resource_type) # AMD Epyc with Nvidia A100. GPU - preemptible instances [gbyte*hour]
            
        # non preemptible
        if(preemptible == False): 
            # cpu
            add_usage("dn2tvhok96voc090hndj", resources_cores*instance_num, resource_name, resource_type) # AMD Epyc with Nvidia A100. 100% vCPU [core*hour]
            # ram
            add_usage("dn24un5470j0l5rbsahu", resources_memory*instance_num, resource_name, resource_type) # AMD Epyc with Nvidia A100. RAM [gbyte*hour]
            # gpu
            add_usage("dn23bfec1dkv6qmeipm7", resources_memory*instance_num, resource_name, resource_type) # AMD Epyc with Nvidia A100. GPU [gbyte*hour]

    if(platform_id == "gpu-standard-v2"):
        # preemptible
        if(preemptible == True):
            # cpu
            add_usage("dn2so61be1pud0fg4mrp", resources_cores*instance_num, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. 100% vCPU - preemptible instances [core*hour]
            # ram
            add_usage("dn2njho0re0917mom5i6", resources_memory*instance_num, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. RAM \u2014 preemptible instances [gbyte*hour]
            # gpu
            add_usage("dn2639t97vtchqueq709", resources_gpus*instance_num, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. GPU - preemptible instances [gbyte*hour]
            
        # non preemptible
        if(preemptible == False):
            # cpu
            add_usage("dn23oh47d817get24dru", resources_cores*instance_num, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. 100% vCPU [core*hour]
            # ram
            add_usage("dn2qiqi4104s7o7eatbp", resources_memory*instance_num, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. RAM [gbyte*hour]
            # gpu
            add_usage("dn2ov44offujr3euo48d", resources_gpus*instance_num, resource_name, resource_type) # Intel Cascade Lake with Nvidia Tesla v100. GPU [gbyte*hour]

    if(platform_id == "gpu-standard-v1"):
        # preemptible
        if(preemptible == True):
            # cpu
            add_usage("dn2g7rssjjnmbl5a3n9b", resources_cores*instance_num, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. 100% vCPU - preemptible instances [core*hour]
            # ram
            add_usage("dn2aohdunrvmqp68d0bf", resources_memory*instance_num, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. RAM - preemptible instances [gbyte*hour]
            # gpu
            add_usage("dn2pr6ebg1947fu35t7a", resources_gpus*instance_num, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. GPU - preemptible instances [gbyte*hour]
            
        # non preemptible
        if(preemptible == False):
            # cpu
            add_usage("dn2fc012e2f1tkd5naci", resources_cores*instance_num, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. 100% vCPU [core*hour]
            # ram
            add_usage("dn25k0fsmikvpi2nbj2m", resources_memory*instance_num, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. RAM [gbyte*hour]
            # gpu
            add_usage("dn2kc7kdofj2bh3boqjd", resources_gpus*instance_num, resource_name, resource_type) # Intel Broadwell with Nvidia Tesla v100. GPU [gbyte*hour]
